...
export async function GET(req: NextRequest) {
  try {
    ...
    const args = { rows, from: parsed.data.from, to: parsed.data.to, lang, type: 'ACCIDENT' as const };
    ...
    if (parsed.data.format === 'excel') {
      ...
      return new Response(new Uint8Array(buf), {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="accidents-${parsed.data.from}-to-${parsed.data.to}.xlsx"`,
        },
      });
    }
    ...
    return new Response(new Uint8Array(buf), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="accidents-${parsed.data.from}-to-${parsed.data.to}.pdf"`,
      },
    });
  } catch (err) {
