# Sales Report App — Master Task Plan v4.0 (매출리포트 앱 마스터 작업계획서)

---
document_id: AMA-SAL-TASK-PLAN-4.0.0
version: 4.0.0
status: In Progress
created: 2026-04-02
updated: 2026-04-02
app: app-sales-report
based_on: AMA-SAL-TASK-PLAN-3.0.0
---

## 1. System Development Status Analysis (시스템 개발 현황 분석)

> **v4.0 Updated**: 2026-04-02 기준 실제 구현 현황 반영

### 1.1 Directory Structure (현재 — 2026-04-02 기준)
...
│   │   ├── raw-order/               ✅ Full — Controller, Service, Parser, CM Report
│   │   │   ├── controller/
│   │   │   │   ├── raw-order.controller.ts      ✅ upload, list, detail, daily-summary
│   │   │   │   └── cm-report.controller.ts      ✅ GET weekly, monthly CM
│   │   │   ├── service/
│   │   │   │   ├── raw-order.service.ts          ✅ uploadExcel, findAll, findOne, dailySummary
│   │   │   │   └── cm-report.service.ts          ✅ getWeeklySummary, getMonthlySummary
│   │   │   ├── parser/
│   │   │   │   ├── excel-parser.interface.ts     ✅ ParsedOrder/ParsedItem interface
│   │   │   │   ├── shopee-excel.parser.ts        ✅ Shopee 68-col XLSX parser
│   │   │   │   └── tiktok-excel.parser.ts        ✅ TikTok 54-col XLSX parser
│   │   │   ├── entity/
│   │   │   │   ├── raw-order.entity.ts           ✅ 40+ columns
│   │   │   │   └── raw-order-item.entity.ts      ✅ 20+ columns
│   │   │   ├── dto/ (request/, response/)        ✅
│   │   │   └── raw-order.module.ts               ✅ Full module
│   │   ├── upload-history/          ✅ Full — Entity, Controller, Service
│   │   │   ├── entity/upload-history.entity.ts   ✅ drd_upload_histories
│   │   │   ├── upload-history.controller.ts      ✅ GET list, GET detail
│   │   │   ├── upload-history.service.ts         ✅ create, complete, fail, findAll, findOne
│   │   │   └── upload-history.module.ts          ✅
│   │   └── product-master-excel/    ✅ Full — Controller, Service
│   │       ├── product-master-excel.controller.ts ✅ template/export/import
│   │       ├── product-master-excel.service.ts    ✅ Excel processing
│   │       └── product-master-excel.module.ts     ✅
│   ├── auth/                    ✅ JWT strategy, guards, @Auth() decorator
│   ├── common/                  ✅ Filters, interceptors, enums
│   ├── app.module.ts            ✅ 8 modules registered
│   └── main.ts                  ✅ Port 3103
├── frontend/src/
│   ├── pages/
│   │   ├── DashboardPage.tsx          ⚠️ Placeholder ("Coming Soon")
│   │   ├── SpuMasterListPage.tsx      ✅ Full CRUD
│   │   ├── SkuMasterListPage.tsx      ✅ Full CRUD + CostHistoryDrawer + Excel toolbar
│   │   ├── ChannelMappingListPage.tsx ✅ Full CRUD
│   │   ├── OrderUploadPage.tsx        ✅ Shopee/TikTok upload + result display
│   │   ├── DailyReportPage.tsx        ✅ Full (date range, channel filter, summary cards, table)
│   │   ├── WeeklyCmPage.tsx           ✅ Full (ISO week accordion, cost breakdown, product CM)
│   │   ├── MonthlyCmPage.tsx          ✅ Full (monthly summary, MoM comparison, product CM)
│   │   ├── RawOrderListPage.tsx       ✅ Full (filters, pagination, detail link)
│   │   ├── RawOrderDetailPage.tsx     ✅ Full (header + items table)
│   │   └── auth/                      ✅ EntityInfo, Login, ChangePassword
│   ├── components/layout/AppLayout.tsx ✅ Sidebar (5 sections, 13 menu items)
│   ├── hooks/                         ✅ useSales, useProductMasterExcel, useRawOrders
│   ├── services/                      ✅ sales.service, product-master-excel.service, raw-order.service
│   ├── stores/                        ✅ auth.store, toast.store
│   ├── lib/                           ✅ api-client, query-client, ama-token
│   └── i18n/locales/{ko,en,vi}/       ✅ sales.json (common, nav, auth, spu, sku, channel, upload, daily, weekly, monthly)
└── docker-compose.app-sales-report.yml ✅
```

### 1.2 Implementation Status by Phase (v3 기준 진행 상태)

| Phase | 범위 | 상태 | 비고 |
|-------|------|------|------|
| Phase 1 | Upload History + Common Utils | ✅ 완료 | entity, module, controller, service 모두 구현 |
| Phase 2 | Product Master Excel | ✅ 완료 | BE template/export/import + FE toolbar 구현 |
| Phase 3 | Order Upload + CM Report | ✅ 완료 | Shopee/TikTok parser, list/detail, CM weekly/monthly |
| Phase 4 | Reference Report Upload (6 types) | ❌ 미구현 | 별도 도메인 모듈 없음 |
| Phase 5 | Build & Deploy | ✅ 완료 | 스테이징 배포 완료 |

### 1.3 Sidebar vs Route 불일치 (사이드바 등록 but 미구현)

| 메뉴 | 라우트 | 페이지 | 상태 |
|------|--------|--------|------|
| Dashboard | `/` | `DashboardPage.tsx` | ⚠️ Placeholder |
| Livestream | `/livestream` | — | ❌ 미구현 (404→리다이렉트) |
| Creator | `/creator` | — | ❌ 미구현 (404→리다이렉트) |
| Manual Input | `/manual-input` | — | ❌ 미구현 (404→리다이렉트) |
| Settings | `/settings` | — | ❌ 미구현 (404→리다이렉트) |
| Notifications | `/notifications` | — | ❌ 미구현 (404→리다이렉트) |

### 1.4 Available Backend APIs

| Endpoint | Method | 구현 |
|----------|--------|------|
| `/api/v1/spu-masters` | CRUD | ✅ |
| `/api/v1/sku-masters` | CRUD | ✅ |
| `/api/v1/channel-masters` | GET | ✅ |
| `/api/v1/channel-product-mappings` | CRUD | ✅ |
| `/api/v1/sku-cost-histories` | GET | ✅ |
| `/api/v1/raw-orders/upload` | POST | ✅ |
| `/api/v1/raw-orders/list` | GET | ✅ |
| `/api/v1/raw-orders/:ord_id` | GET | ✅ |
| `/api/v1/raw-orders/daily-summary` | GET | ✅ |
| `/api/v1/raw-orders/upload-history` | GET | ✅ |
| `/api/v1/cm-report/weekly` | GET | ✅ |
| `/api/v1/cm-report/monthly` | GET | ✅ |
| `/api/v1/upload-histories` | GET | ✅ |
| `/api/v1/upload-histories/:uph_id` | GET | ✅ |
| `/api/v1/product-master/template` | GET | ✅ |
| `/api/v1/product-master/export` | GET | ✅ |
| `/api/v1/product-master/import` | POST | ✅ |
| `/api/v1/raw-orders/dashboard-summary` | GET | ❌ 신규 추가 필요 |

### 1.5 Constraints & Observations

1. **Phase 1~3 all completed** — 8개 BE 모듈, 10개 FE 페이지 구현
2. **xlsx + multer already installed** — no dependency installation phase needed
3. **Dashboard는 BE API 신규 필요** — KPI 집계 API (전체 주문/GMV/CM 요약)
4. **5개 사이드바 메뉴 미구현** — Placeholder 페이지 또는 제거 필요
5. **DB_SYNC=false on staging** — 새 테이블은 수동 SQL 또는 docker recreate 필요

---

## 2. Implementation Plan — Remaining Work (남은 작업 구현 계획)

> Phase 1~3, Phase 5는 완료됨. 아래는 **미구현 항목만** 정리.

### Phase A: Dashboard Real Data (대시보드 실데이터) — P0

#### Step A.1: Backend — Dashboard KPI API
- Add method in `raw-order.service.ts`: `getDashboardSummary(entId)`
  - 전체 주문수, GMV, CM 요약 (최근 30일)
  - 채널별 GMV 비중 (SHOPEE vs TIKTOK)
  - 최근 7일 일별 GMV 추이 라인 데이터
  - SKU 매칭률 + 최근 업로드 이력
- Add endpoint in `raw-order.controller.ts`: `GET /api/v1/raw-orders/dashboard-summary`
- `└─ Side effect: raw-order.controller.ts, raw-order.service.ts 수정`

#### Step A.2: Frontend — Dashboard page rewrite
- Rewrite `DashboardPage.tsx` from Placeholder → real KPI dashboard:
  - 4개 KPI 카드: 총주문, 총GMV, CM총액, CM%
  - 채널별 GMV 도넛 차트 (recharts)
  - 최근 7일 GMV 추이 라인 차트 (recharts)
  - 최근 업로드 이력 요약
  - SKU 매칭률 진행바
- Add React Query hook: `useDashboardSummary()`
- i18n keys: `dashboard.*`
- `└─ Side effect: DashboardPage.tsx 전체 교체, i18n 추가`

### Phase B: Upload History Page (업로드 이력 페이지) — P1

#### Step B.1: Frontend — Upload History page
- Create `pages/UploadHistoryPage.tsx`:
  - 테이블: 날짜, 유형, 채널, 파일명, 행수, 성공, 에러, 상태 뱃지
  - 페이지네이션
  - 행 클릭 → 에러 상세 모달
- Add route: `/upload-history`
- Add sidebar nav under "데이터입력" section
- i18n keys: `uploadHistory.*`
- `└─ Side effect: App.tsx route, AppLayout sidebar, i18n`

### Phase C: Placeholder Pages for Unimplemented Routes — P1

#### Step C.1: Create placeholder pages for sidebar routes
- 5개 미구현 라우트를 Placeholder 페이지로 추가 (404 방지):
  - `pages/LivestreamPage.tsx` → "Coming Soon"
  - `pages/CreatorPage.tsx` → "Coming Soon"
  - `pages/ManualInputPage.tsx` → "Coming Soon"
  - `pages/SettingsPage.tsx` → "Coming Soon"
  - `pages/NotificationsPage.tsx` → "Coming Soon"
- Add routes to `App.tsx`
- `└─ Side effect: App.tsx route 추가`

### Phase D: Reference Report Upload (참고 보고서 업로드) — P2 (Future)

> 6개 참조 리포트 타입 (트래픽/광고/제휴) 업로드는 Phase D로 이관.
> 별도 세션에서 진행 예정 — 엔티티 + 파서 + FE 탭 확장.
> v3 계획서의 Phase 4 내용을 그대로 유지.
