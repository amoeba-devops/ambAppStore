...
export function DriverFormModal({ open, onClose, vehicleId }: DriverFormModalProps) {
  ...
  // Auto-switch to manual mode if OAuth not connected
  useEffect(() => {
    if (open && oauthData && !oauthConnected) {
      setManualMode(true);
    }
  }, [open, oauthData, oauthConnected]);

  const resetForm = () => {
    setManualMode(!oauthConnected);
    setManualUuid('');
    setSearchQuery('');
    setDebouncedSearch('');
        ...
        <div>
          ...
          {manualMode ? (
            ...
            <div className="flex items-center gap-2 rounded-lg border border-orange-300 bg-orange-50 px-3 py-2">
              ...
              <button type="button" onClick={() => { setSelectedMember(null); setSearchQuery(''); }} className="flex-shrink-0 rounded p-0.5 text-gray-400 hover:bg-orange-100 hover:text-gray-600">
                <X className="h-4 w-4" />
              </button>
            </div>
          ) : (
            ...
            <div ref={dropdownRef} className="relative">
              <div className="relative">
                ...
                />
                {membersLoading && <Loader2 className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-gray-400" />}
              </div>
