...
const baseUrl = process.env.DEV_WEB_URL ?? 'http://localhost:3000';
...
const token = await new SignJWT({
  ...
  role,
  email: `dev-${role.toLowerCase()}@dev.firgi.local`,
  name: `Dev ${role}`,
...
console.log('');
console.log(`Role: ${role}  →  local: ${role === 'OWNER' || role === 'MASTER' ? 'ADMIN' : role === 'MANAGER' ? 'MANAGER' : 'OPERATOR'}`);
