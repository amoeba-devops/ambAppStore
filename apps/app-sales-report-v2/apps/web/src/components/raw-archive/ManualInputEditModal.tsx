...
type GroupKey = 'totalPlatform' | 'shopee' | 'tiktok';

/** Field grouping for nicer layout — mirrors Step 3 wizard structure. */
function groupOf(field: string): GroupKey {
  if (field.startsWith('Total Affiliate Booking Fee') && !field.includes('—')) return 'totalPlatform';
...
export function ManualInputEditModal({ periodKey, periodLabel, initial, snapshotRef, onClose }: Props) {
  const t = useTranslations('rawArchive.manualEdit');
      ...
      >
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              <h3 className="text-sm font-semibold text-neutral-900 font-mono leading-tight mt-0.5">
                {t('periodLabel', { label: periodLabel })}
              </h3>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {grouped.map(([groupKey, groupFields]) => (
            <div key={groupKey} className="rounded-md border border-neutral-200 overflow-hidden">
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        <div className="text-xs text-neutral-700 truncate">{manualFieldLabel(f)}</div>
                        {changed && (
                          <div className="text-[10px] text-neutral-500 tabular-nums">
                            {t('wasValue', { value: fmtVND(baseline) })}
                          </div>
                        )}
                      </div>
          ...
          <span className="text-[11px] text-neutral-500">
            {changedFields.length === 0
              ...
              : changedFields.length === 1
                ? t('changedCountSingular')
                : t('changedCount', { count: changedFields.length })}
          </span>
