...
export default async function AuditLogPage() {
  ...
  return (
    <AppFrame
      title={t('title')}
      subtitle={t('subtitle')}
      breadcrumbs={[tCo('tenant'), tNav('auditLog')]}
      actions={<Btn kind="ghost" icon="download">{tA('export')} CSV</Btn>}
    >
      <PageContent>
        <PhaseGate
          phase="P1"
          ...
          skeleton={
            <ListPageShell>
              <EmptyState icon="audit" tone="info" title={`${tP('comingIn')} P1`} body={tP('phasePlan')} />
            </ListPageShell>
          }
        />
      </PageContent>
