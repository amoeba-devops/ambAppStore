...
export default async function NewTruckPage() {
  ...
  const [drivers, regionOptions] = await Promise.all([
    listFleetDrivers(user.entId, 'TRUCK').then((ds) =>
      ds.map((d) => ({ id: d.drvId, name: driverIdentity(d) })),
    ),
    allowedRegions(user),
  ]);
  ...
  return (
    <>
      ...
      <div className="px-4 md:px-7 py-4 md:py-6 max-w-2xl mx-auto w-full">
        <TruckVehicleForm drivers={drivers} regionOptions={regionOptions} />
      </div>
    </>
  );
}
