...
export default async function EditTruckTripPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [vehicles, drivers, extras] = await Promise.all([
  ...
  const initial = {
    ...
    startOdo: trip.trpStartOdometer != null ? String(trip.trpStartOdometer) : '',
    endOdo: trip.trpEndOdometer != null ? String(trip.trpEndOdometer) : '',
  ...
  return (
    <>
      ...
      <div className="px-4 md:px-7 py-4 md:py-6 max-w-3xl mx-auto md:mx-0 w-full">
        <TruckTripForm vehicles={vehicleOptions} drivers={driverOptions} tripId={trip.trpId} initial={initial} />
      </div>
    </>
  );
}
