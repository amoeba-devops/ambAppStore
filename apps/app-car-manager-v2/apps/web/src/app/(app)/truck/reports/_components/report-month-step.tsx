 ...
 */
export function ReportMonthStep({ exportedMonths }: { exportedMonths: string[] }) {
  ...
  const cont = () => {
    if (selected) router.push(`/truck/reports/new?month=${selected}`);
  };
    ...
    <div className="space-y-5">
      ...
      <Card variant="outline" className="space-y-4 p-5">
          ...
          {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => {
            ...
            return (
              <button
                ...
                onDoubleClick={() => router.push(`/truck/reports/new?month=${key}`)}
        ...
        <div className="flex justify-end">
          <Button variant="accent" size="md" onClick={cont} disabled={!selected}>
            {tA('continue')}
            <ChevronRight className="h-4 w-4" />
          </Button>
        </div>
