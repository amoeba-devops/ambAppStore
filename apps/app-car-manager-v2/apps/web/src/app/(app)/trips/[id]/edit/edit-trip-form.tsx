...
export function EditTripForm({
  ...
  /* Conflict check active khi có cả driver + vehicle. Dùng giá trị state hiện
   * tại (admin có thể đang đổi assignment) để re-check theo input mới. */
  ...
  const { conflicts, loading: conflictsLoading, refetch: refetchConflicts } = useTripConflicts({
    ...
    enabled: Boolean(vehicleId && driverId),
  });
